    ...
    while let Some(mut b) = try!(iter.next_box()) {
        match b.head.name {
            BoxType::EditListBox => {
                ...
                track.media_time = Some(TrackScaledTime {
                    time: elst.edits[idx].media_time as u64,
                    id: track.id,
                });
                log!("{:?}", elst);
            }
...
fn parse_mdhd<T: Read>(f: &mut BMFFBox<T>, track: &mut Track) -> Result<(MediaHeaderBox, Option<TrackScaledTime>, Option<TrackTimeScale>)> {
    ...
    let timescale = Some(TrackTimeScale {
        scale: mdhd.timescale as u64,
        id: track.id,
    });
